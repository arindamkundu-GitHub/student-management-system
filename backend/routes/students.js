...
router.route("/add").post(async (req, res) => {
    try {
        const { firstName, lastName, regNumber, age, gender, address, contactNumber,email,enrolledDate } = req.body;
...
router.route("/update/:id").put(async (req, res) => {
    try {
        ...
        const update = await Student.findByIdAndUpdate(userId, updateStudent,{ new: true });
        res.status(200).send({status: "User Updated",data: update});
    } catch (err) {
        console.log(err);
